      ...
      <div style="text-align:center">
        <form method="post" name="form1" id="form1" action="delete.php">
          <table border="1px" style="margin:32px">
            ...
            <tbody>
              <?php
                $sql="SELECT id,role FROM Account WHERE id!='$user'";
                if($result= mysqli_query($link, $sql)){
                  ...
                  ?>
                  <tr>
                    <td width='15%' style="text-align: center;"><input type="checkbox" name="delete[]" value="<?php echo $id;?>" id="<?php echo $id;?>"></td>
                    <td width='40%'><?php echo $id;?></td>
                    <td width='86%'>
                      <select size=1 name="select" onchange="location = this.options[this.selectedIndex].value;">
                        <option value="<?php echo "edit.php?id=".$id.'&role=0'?>" <?php if($role=='General member') echo "selected"?>>General member
                        <option value="<?php echo "edit.php?id=".$id.'&role=1'?>" <?php if($role=='Premium member') echo "selected"?>>Premium member
                        <option value="<?php echo "edit.php?id=".$id.'&role=2'?>" <?php if($role=='Administrator') echo "selected"?>>Administrator
                      </select>
                    </td>
                  </tr>
                <?php
